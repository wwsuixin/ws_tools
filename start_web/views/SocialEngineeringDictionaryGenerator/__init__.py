...
tool_info = SocialEngineeringDictionaryGenerator_INFO  # 工具信息
...
def SocialEngineeringDictionaryGenerator_exec(url_file="", plugins="", output_name=""):
    shell = ""
    dir_name, full_file_name = os.path.split(tool_info["path"])
    shell += f"""cd '{dir_name}'; """
    shell += f"""{tool_info['run_exec']}  .\{full_file_name};"""
    shell += "\n"  # 拆分为多条命令同时执行,以回车分割
    return shell
...
def index():
    ...
    return render_template(
        ...
        res=dict(
            ...
            down_url=down_url,
            form_run="true",  # 运行按钮
            shell=SocialEngineeringDictionaryGenerator_exec(),
        ),
    )

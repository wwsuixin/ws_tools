...
config_file_name = f"fofa.ini"  # 配置文件名称
...
def update():
    ...
    ip_mask_str = post_data.get("ip_mask_str")
    domain_str = post_data.get("domain_str")
    icp_str = post_data.get("icp_str")

    domain_list = list(domain_str.split("\n"))
    icp_list = list(icp_str.split("\n"))
    ip_mask_list = list(ip_mask_str.split("\n"))

    domain_list = [x.strip() for x in domain_list]
    icp_list = [x.strip() for x in icp_list]
    ip_mask_list = [x.strip() for x in ip_mask_list]
    ...
    for domain in domain_list:
        if (
            len(domain) == 0
        ):  # 解决bug：['domain=""', 'cert=""', 'host=""', 'icp=""', 'ip=""']
            continue
        res_list.append(f'''domain="{domain}"''')
        res_list.append(f'''cert="{domain}"''')
        res_list.append(f'''host="{domain}"''')
    for icp in icp_list:
        if len(icp) == 0:
            continue
        res_list.append(f'''icp="{icp}"''')
    for ip_mask in ip_mask_list:
        if len(ip_mask) == 0:
            continue
        res_list.append(f'''ip="{ip_mask}"''')
    # print(f"{res_list}")
    res_str = ""
    if len(res_list) > 0:
        # 旧版本文件引入方式
    ...
    res["data"] = fofamap_exec(
        target_file_path=res_str, output_file_path=output_file_path
    )
    return res
...
def fofamap_exec(target_file_path="", plugins="", output_file_path=""):
    shell = ""

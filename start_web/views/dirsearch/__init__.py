...
def update():
    ...
    res["data"] = dirsearch_exec(target_file_path=target_file_path, output_file_path=output_file_path)
    return res
...
def save_diy_dict():
    ...
    if file_content:
        ...
        with open(file=diy_dict_file_path, mode="w", encoding="utf-8") as fw:
            fw.write("\n".join(自定义保存字典列表))
    return res
def dirsearch_exec(target_file_path="", plugins="", output_file_path=""):
    shell = ""

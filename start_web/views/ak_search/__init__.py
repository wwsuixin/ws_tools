...
def update():
    ...
    output_file_path = f"""{os.path.join(res_output_path,f"{output_time}")}.xlsx"""

    res["data"] = ak_search_exec(target_file_path=targets, output_file_path=output_file_path)
    return res
...
def tool_init():
    ...
    return res
def ak_search_exec(target_file_path="", plugins="", output_file_path=""):
    shell = ""

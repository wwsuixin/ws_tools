...
# urlparse(url)
# Out[3]: ParseResult(scheme='https', netloc='blog.csdn.net', path='/yilovexing/article/details/96432467', params='', query='', fragment='')

# In [4]: urlparse(url).netloc
# Out[4]: 'blog.csdn.net'


def nacos_exec(url_file="", plugins="", output_name=""):
    try:
        dir_name, full_file_name = os.path.split(tool_path)
        ...
        with open(url_file, "r", encoding="utf-8") as fr:
            urls_list = fr.read().splitlines()
        for url_str in urls_list:
            url_info = urlparse(url_str)
            url = f"""{url_info.scheme}://{url_info.netloc}"""
            shell += f""" .\{full_file_name} -t '{url}' -s '{output_path_dir}';"""
        shell += rf"""  explorer {output_path_dir};"""
        return shell
    except Exception as e:
        return str(e)

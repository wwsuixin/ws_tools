...
def update(data=""):
    ...
    ips = post_data.get("targets").strip()

    # 定义变量
    ip_list = ips.splitlines()
    result_list = []
    for i in ip_list:
        result_list += ip_create(i)
    result_list = list(set(result_list))
    with open(
        file=os.path.join(res_output_path, "ip_create_result.txt"),
        mode="w",
        encoding="utf-8",
    ) as fw:
        fw.write("\n".join(result_list))
